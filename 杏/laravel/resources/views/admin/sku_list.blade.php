    ...
    <thead> 
     ...
     @foreach($bookdata as $val)
      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
        ...
        <td align="center" valign="middle" class="borderright borderbottom">{{$val['book_stock']}}</td>
        <td align="center" valign="middle" class="borderright borderbottom">@if($val['book_status']==0)
                  下架
                    @else
                    上架    
                    @endif
        <td align="center" valign="middle" class="borderbottom"><a href="{{url('AdminIndex/skudel',['book_id' => $val['book_id']])}}" target="mainFrame" onFocus="this.blur()" class="add">删除</a><span class="gray">&nbsp;|&nbsp;</span><a href="{{url('AdminIndex/skuup',['book_id' => $val['book_id']])}}" target="mainFrame" onFocus="this.blur()" class="add">编辑</a></td>
      </tr>
       @endforeach 
     </tbody>
    </table>
